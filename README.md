...
I used **python 3.7** and the following libraries:   
...
Input in LTSM is a 3D tensor with shape: _[batch, timesteps, feature]_. Reshape input data into 3D format. Input is the price of a previous day.
...
Build **Sequential model**, which is a linear stack of layers: a single **LSTM hidden layer** with 5 memory blocks or neurons, followed by **densely-connected layer** with 1 neuron to output forecast.

Input data specified in LSTM layer must have  3D dimention:  _[batch_size, time steps, features]_, where _batch_size_ is the number of samples shown to Neural Network before updating the weights. The size of training and test datasets must be divisible by _batch_size_ without a remainder. Set _n_batch=1_, i.e. update weights every timestep.

Make LSTM stateful by setting _stateful=True_ to avoid clearing the state between batches. 
...
Set _shuffle=False_ to disable shuffling  samples prior to being exposed to the network.
```
...
Predicted values are in a scale of LSTM activation function, i.e. tanh. Function _invert_scale_ returns forecasts to the original scale, using **inverse_transform** method. 
Because data were scaled before spliting into input (x) and output(y), **invert_transform** requires input shape to be _[n_samples,2]_. Concatenate input values with values predicted by a model.
```
...
```
After inverting to the original scale, predicted values are still need to be transformed, because they're differenced values. Function  _invert_diff_  invert differenced values back to a form of prices.
```
